...
static void drawBoard(const IBoardView& b, std::optional<Pos> last, std::optional<Pos> cursor)
{
    clearScreen();
...
static void runGame(Mode mode, bool humanBlack)
{
    ...
    RuleSet rules {};
    SessionController session { rules, Controller::Human, Controller::Human };
    ...
    // Si l'IA commence
    auto ctrl = [&](Player p) { return session.controller(p); };
